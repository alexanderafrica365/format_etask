...
/**
 * This method finds all courses in 'etask' format that have actual number of sections different than their 'numsections' course
 * format option.
 *
 * For courses where there are more sections than numsections, we call {@link format_etask_upgrade_hide_extra_sections()} and
 * either delete or hide "orphaned" sections. For courses where there are fewer sections than numsections, we call
 * {@link format_etask_upgrade_add_empty_sections()} to add these sections.
 *
 * @return void
 */
function format_etask_upgrade_remove_numsections(): void {
    global $DB;
    ...
    foreach ($needsections as $courseid => $numsections) {
        format_etask_upgrade_add_empty_sections($courseid, $numsections);
    }
...
/**
 * Find all sections in the course with sectionnum bigger than numsections. Either delete these sections or hide them.
 *
 ...
 * @return void
 */
function format_etask_upgrade_hide_extra_sections(int $courseid, int $numsections): void {
    global $DB;
...
/**
 * This method adds empty sections to courses which have fewer sections than their 'numsections' course format option and adds
 * these empty sections.
 *
 * @param int $courseid
 * @param int $numsections
 *
 * @return void
 */
function format_etask_upgrade_add_empty_sections(int $courseid, int $numsections) {
    global $DB;
